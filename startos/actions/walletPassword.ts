...
import { startupFlagsJson } from '../fileModels/startupFlags.json' // Added for restore flag
import { sdk } from '../sdk'
import type { Effects } from '@start9labs/start-sdk/base/lib/types'
import { lndDataDir, mainMounts } from '../utils'
import { restPort } from '../interfaces' // Added for dynamic port
import { base64 } from 'rfc4648' // Added for upstream's fixed encoding
...
export const manualWalletUnlock = sdk.Action.withInput(
  ...
  async ({ effects, input }: { effects: Effects; input: ManualUnlockInput }) => {
    ...
    // Use upstream's fixed base64 encoding for the LND API
    const walletPasswordBase64 = base64.stringify(Buffer.from(password, 'latin1'))
    console.log('Unlocking wallet with provided password...')
    ...
    try {
      const res = await sdk.SubContainer.withTemp(
        ...
        'manual-unlock-temp',
        async (lndSub) => {
          // Read restore flag from the new startup-flags.json instead of store.json
          ...
          return await lndSub.exec([
            ...
            '--cacert',
            `${lndDataDir}/tls.cert`,
            `https://lnd.startos:${restPort}/v1/unlockwallet`, // Dynamic port
            '-d',
            currentRestore
              ? JSON.stringify({
                  wallet_password: walletPasswordBase64,
                  recovery_window: 2500, // Hardcoded like upstream
                })
              : JSON.stringify({
                  wallet_password: walletPasswordBase64,
                }),
      ...
      if (res.stdout === '{}' && res.exitCode === 0) {
        ...
        <h3 class="g-secondary"><br><img src="data:image/svg+xml;base64," alt="stopwatch-duotone" height="32" width="32">&nbsp;&nbsp;Your wallet is ready to use. Health checks will update "Wallet Status" within 30 seconds.<br><br></h3></span>`,
          result: null,
        }
      } else {
...
export const walletPassword = sdk.Action.withInput(
  ...
  async ({ effects }) => {
    ...
    if (autoUnlockEnabled && store?.walletPassword) {
      // FIX: store.walletPassword is already plaintext! No need to decode from base64.
      currentPasswordDefault = store.walletPassword
      currentPasswordDescription = 'Your current wallet password (loaded from store).'
      console.log('Pre-filling current password field for user convenience.')
    } else {
      console.log('Auto-unlock disabled or no password in store. Leaving current password field empty.')
      currentPasswordDescription = 'Your current wallet password (enter manually).'
    }
  ...
  async ({ effects, input }: { effects: Effects; input: Input }) => {
    ...
    // We store the pending password in base64 just as a safe transport mechanism to main.ts
