...
export const deleteCipherSeed = sdk.Action.withoutInput(
  ...
  async ({ effects }) => {
    ...
    return {
      ...
      description:
        'Delete the Aezeed Cipher Seed from the server (store.json). If not deleted, anyone with physical access to the server can reflash StartOS, set a new master password, convert the Aezeed seed to an HD key, and then import that key into an external wallet to steal your on-chain funds.',
      warning:
